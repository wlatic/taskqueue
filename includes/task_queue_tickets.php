...
if(!class_exists('TaskQueueTickets'))
{
    class TaskQueueTickets
    {
        ...
        {
            add_filter('get_opened_tickets', array($this, 'get_opened_tickets'));
            add_filter('get_closed_tickets', array($this, 'get_closed_tickets'));
            
            $this->enqueue_scripts();
            
            load_template(__DIR__ . '/task_queue/index.php');
        }

        public function add_task_ticket()
        {
            $class = new TaskQueueTickets();
            $class->enqueue_scripts();
            load_template(__DIR__ . '/task_queue/add_new.php');
        }
        ...
        {
            return $this->fetch_tickets('Open');
        }
        ...
        public function view_task_ticket($ticket_id)
        {
            $class = new TaskQueueTickets();
            $class->enqueue_scripts();
            ...
            // $class->enqueue_scripts();
            load_template(__DIR__ . '/task_queue/view_ticket.php');
        }

        public function add_reply($ticket_id, $reply)
        {
            global $current_user;
            $class = new TaskQueueTickets();
            ...
            if(wp_redirect(get_page_link(get_page_by_title('Task Queue Tickets'))))
            {
                exit();
            }
        }
        ...
        public function delete_task_ticket($ticket_id)
        {
            $class = new TaskQueueTickets();
        ...
        public function new_task_ticket($ticket_data, $attachment)
        {
            global $current_user;
            $class = new TaskQueueTickets();
            // echo "Location: " . get_page_link(get_page_by_title('Task Queue Tickets'));exit();
            ...
            $max_sortorder = $class->get_max_sortorder();
            ...
            $ticket = array(
                    ...
                    'custom_fields[cf_sortorder]' => $sortorder
            );
                ...
                if (( isset($this->opt['freshdesk_apikey']) && $this->opt['freshdesk_apikey'] != '' ) || !isset($this->opt['use_apikey'])) {
                    ...
                    if (isset($tickets)) {
                        ...
                        if (isset($status) && $status != '') {
                            $filteredTickets = ( $status != 'all_tickets' ) ? $this->filter_tickets($tickets, $status, $comparison) : $tickets ;
            ...
            if(!isset($tickets['errors']))
            {
                if ($status != 'all_tickets') {
                    foreach ($tickets as $t) {
                        if($comparison == '!=')
                        {
                            if ($t['status_name'] != $status && $t['group_id'] == $task_group_id) {
                                $filtered_tickets[] = $t;
                            }
                        }
                        else
